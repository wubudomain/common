package top.wboost.common.base.core;
...
public class ExecutorsDaemon {
    ...
    public static ThreadFactory getDaemonThreadFactory(String poolName) {
        return new DefaultDaemonThreadFactory(poolName);
    }
    ...
    static class DefaultDaemonThreadFactory implements ThreadFactory {
        ...
        DefaultDaemonThreadFactory(String poolName) {
            SecurityManager s = System.getSecurityManager();
            group = (s != null) ? s.getThreadGroup() : Thread.currentThread().getThreadGroup();
