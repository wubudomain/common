...
public class StringUtil {
	...
	private static String bytes2Hex(byte[] b) {
		/*
		 * byte[] buff = new byte[3 * b.length]; for (int i = 0; i < b.length;
		 * i++) { buff[3 * i] = HEX_BYTE[(b[i] >> 4) & 0x0f]; buff[3 * i + 1] =
		 * HEX_BYTE[b[i] & 0x0f]; buff[3 * i + 2] = 45; } return new
		 * String(buff);
		 */
	...
	/**
	 * 将有空格/换行符等格式化为一个空格，转为标准一句话，如 SELECT log_id AS logid, title AS titleabc
	 * FROM t_sys_log
	 * 
	 * @param sql
	 * @return
	 ...
	 * @return
	 */
	public static String getByLowerBegin(String name) {
