...
public class EncodingDetect {
    ...
    public static String getJavaEncode(byte[] bytes) {
        return Encoding.javaname[new BytesEncodingDetect().detectEncoding(bytes)];
    }

    public static String getJavaEncode(File file) {
        return Encoding.javaname[new BytesEncodingDetect().detectEncoding(file)];
    }
    ...
    public static String readFile(String file, boolean... isTrims) {
        ...
        try {
            ...
            while ((line = fr.readLine()) != null) {
                ...
                flag++;
                buffer.append(isTrim ? StringUtil.notEmpty(line) : line);
            }
            fr.close();
            ...
            if (rawtext[i] >= 0) {
                ...
                if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF7 && // Original GB range
                    ...
                    if (GBKFreq[row][column] != 0) {
                        ...
                        && (byte) 0x81 <= rawtext[i + 2] && rawtext[i + 2] <= (byte) 0xFE && (byte) 0x30 <= rawtext[i
                        + 3] && rawtext[i + 3] <= (byte) 0x39) {
                    gbchars++;
                    /*
...
class Encoding {
    // Suc final static int SIMP = 0;

    public final static int TRAD = 1;

    // lipported Encoding Types
