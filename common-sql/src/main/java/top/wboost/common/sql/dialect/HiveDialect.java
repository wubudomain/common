    ...
    private void initLimitHandler(SqlParameterEncoder encoder) {
        this.LIMIT_HANDLER = new AbstractRealSqlLimitHandler(encoder) {
            ...
            public String processSql(String sql, RowSelection selection) {
                String beginRow;
                String endRow;
                beginRow = encoder.encode(selection.getFirstRow() + "");
                endRow = encoder.encode((selection.getFetchSize() + selection.getFirstRow()) + "");
                return "select * from ( select *,ROW_NUMBER() over() as rnum from (" + sql + ") where rnum between "
                        + beginRow + " and " + endRow;
            }
